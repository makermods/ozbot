...
function getLevelTierTable(level) {
  const entry = LEVEL_TIERS.find(t => level >= t.min && level <= t.max);
  const table = entry?.values || LEVEL_TIERS[0].values;
  log(`📘 [TIER TABLE] Using tier table for level ${level}: [${table.join(', ')}]`);
  return table;
}
